...
RUN printf '#!/bin/bash\nset -e\nCONN="Host=$(printenv '"'"'Postgres.PGHOST'"'"');Port=$(printenv '"'"'Postgres.PGPORT'"'"');Database=$(printenv '"'"'Postgres.PGDATABASE'"'"');Username=$(printenv '"'"'Postgres.PGUSER'"'"');Password=$(printenv '"'"'Postgres.PGPASSWORD'"'"');Pooling=True;Minimum Pool Size=1;Maximum Pool Size=1024"\nSETTINGS_FILE="/app/App_Data/Tenants/Default/Settings.txt"\nif [ ! -f "$SETTINGS_FILE" ]; then\n  if [ -z "$(printenv '"'"'Postgres.PGHOST'"'"')" ]; then\n    echo "ERROR: Settings.txt not found and Postgres.PGHOST is not set."\n    exit 1\n  fi\n  mkdir -p "$(dirname "$SETTINGS_FILE")"\n  printf "AppVersion: ${SMARTSTORE_APP_VERSION:-6.3.0.0}\\nDataProvider: ${SMARTSTORE_DB_PROVIDER:-PostgreSql}\\nDataConnectionString: ${CONN}\\n" > "$SETTINGS_FILE"\nfi\nexec ./Smartstore.Web --urls http://0.0.0.0:80\n' > entrypoint.sh && chmod +x entrypoint.sh
